...
export async function GET(
  ...
  if (!z.uuid().safeParse(id).success)
    return failure("Không tìm thấy bài đăng.", 404);
  ...
  if (postRes.error)
    return failure("Chưa tải được bài đăng.", 503, { dbError: postRes.error });
  ...
  if (!post.success || !comments.success)
    return failure("Dữ liệu bài đăng chưa đúng định dạng.", 502);
...
export async function POST(
  ...
  if (!parsed.success)
    return failure(parsed.error.issues[0]?.message ?? "Thao tác không hợp lệ.");

  if (parsed.data.action === "react") {
    const { data, error } = await db.rpc("toggle_local_post_reaction", {
      p_id: id,
    });
    if (error)
      return databaseFailure(error, { entity: "local_post", action: "react" });
    return ok(data);
  }
  if (parsed.data.action === "comment") {
    const { data, error } = await db.rpc("comment_local_post", {
      p_id: id,
  ...
  const { error } = await db.rpc("report_local_post", {
    p_id: id,
    p_reason: parsed.data.reason ?? "",
  });
  if (error)
    return databaseFailure(error, { entity: "local_post", action: "report" });
