...
export async function GET(request: Request) {
  ...
  const { data, error } = await db.rpc("discover_local_posts", { p_bounds: bounds.data });
  if (error) return failure("Chưa tải được bài địa phương.", 503, { dbError: error });
  const parsed = localPostSchema.array().safeParse(data);
  if (!parsed.success) return failure("Dữ liệu bài địa phương chưa đúng định dạng.", 502, { parseErrors: parsed.error.issues });
  return ok({ mode: "live", posts: parsed.data, truncated: parsed.data.length === 100 });
}

export async function POST(request: Request) {
  const db = requestSupabase(request);
  if (!db) return failure("Bản xem thử không lưu bài đăng. Cần kết nối Supabase.", 503);
  ...
  if (!parsed.success) return failure(parsed.error.issues[0]?.message ?? "Bài đăng không hợp lệ.");

  const { data, error } = await db.rpc("create_local_post", { p_input: parsed.data });
