...
export async function GET(
  ...
  if (!z.uuid().safeParse(id).success) return failure("Không tìm thấy hồ sơ.", 404);
  ...
  if (!parsed.success) return failure("Dữ liệu hồ sơ chưa đúng định dạng.", 502, { parseErrors: parsed.error.issues });
...
export async function POST(
  ...
  const { data, error } = await db.rpc("set_follow", { p_user_id: id, p_follow: parsed.data.follow });
  if (error) return databaseFailure(error, { entity: "profile", action: parsed.data.follow ? "follow" : "unfollow" });
  return ok(data);
}
